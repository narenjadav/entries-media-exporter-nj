...
class GravityForms {
	...
	public function seed_sample_entries( int $form_id, int $count ) {
		...
		if ( ! empty( $uploads['error'] ) ) {
			return new WP_Error( 'uploads', __( 'Uploads directory is not writable: ', 'entries-media-exporter-nj' ) . $uploads['error'] );
		}

		$dir_path = trailingslashit( $uploads['basedir'] ) . 'emenj-samples';
		$dir_url  = trailingslashit( $uploads['baseurl'] ) . 'emenj-samples';
		if ( ! wp_mkdir_p( $dir_path ) ) {
			return new WP_Error( 'mkdir', __( 'Could not create the sample files directory.', 'entries-media-exporter-nj' ) );
		}
		...
		if ( 0 === $created ) {
			return new WP_Error( 'none', __( 'No sample entries could be created. Check the debug log.', 'entries-media-exporter-nj' ) );
		}
		return $created;
	}
